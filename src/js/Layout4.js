...
/*function filterElementsByType(flowElements, type) {
    return flowElements.filter(e => {
        //console.log(e.id != id)
        return e.$type != type
    })
    ...
    while (flowElements.length > 0) {
        ...
        oldNext.forEach(e => {
            ...
            if (e.$type != "bpmn:EndEvent") {
                nextFlowElements = []
                newFlowElements, nextFlowElements, deleteFlowElements = this._test2(newFlowElements, nextFlowElements, e, deleteFlowElements)
                deleteFlowElements.forEach(element => {
                    flowElements = filterElementsById(flowElements, element);
                });
            }
