 ...
 */
  ...
  */
  ...
  BaseViewer.prototype.getPoint = function (line, shape, type) {
    var centx;
    var centy;
    var x;
    var y;
    if(type == 'source'){
      ...
      if(line.sourceSide == 'left'){
        x = centx - shape.bounds.width/2
        y = centy
      }
      if(line.sourceSide == 'right'){
        x = centx + shape.bounds.width/2
        y = centy
      }
      if(line.sourceSide == 'up'){
        x = centx;
        y = centy - shape.bounds.height/2
      }
      if(line.sourceSide == 'down'){
        x = centx;
        y = centy + shape.bounds.height/2
      }
    }
    if(type == 'target'){
      centx = shape.bounds.x + shape.bounds.width/2
      centy = shape.bounds.y + shape.bounds.height/2
      if(line.targetSide == 'left'){
        x = centx - shape.bounds.width/2
        y = centy
      }
      if(line.targetSide == 'right'){
        x = centx + shape.bounds.width/2
        y = centy
      }
      if(line.targetSide == 'up'){
        x = centx;
        y = centy - shape.bounds.height/2
      }
      if(line.targetSide == 'down'){
        x = centx;
        y = centy + shape.bounds.height/2
      }
    }
  ...
  BaseViewer.prototype.getSide = function (line, shape, type) {
    var first;
    var centx;
    var centy;
    var last;
  ...
  BaseViewer.prototype.func3 = function (elements) {
    var source;
    var target;
    var i = 0;
    var side;
      ...
      if(element.waypoint){
        ...
        while(i < elements.length){
          if(source == elements[i].bpmnElement.id){
            // Se busca el lado de encuentro
            side = this.getSide(element, elements[i], 'source');
            element.sourceSide = side;
            i = elements.length
          }
          i += 1
        }
        i = 0
        target = element.bpmnElement.targetRef.id
        while(i < elements.length){
          if(target == elements[i].bpmnElement.id){
            // Se busca el lado de encuentro
            side = this.getSide(element, elements[i], 'target');
            element.targetSide = side;
            i = elements.length
          }
          i += 1
        }
  ...
  BaseViewer.prototype.func2 = function (elements) {
    var i = 0;
    while (i < elements.length) {
      ...
      if (element.bounds) {
        ...
        elements.forEach(e => {
          if (e.bounds) {
            if (element.bounds.x >= e.bounds.x) {
              e.bounds.x += 30
            }
          }
          if (e.waypoint) {
            e.waypoint.forEach(el => {
              if(element.bounds.x >= el.x){
                el.x += 30
              }
            });
        }
  ...
  BaseViewer.prototype.func = function (elements) {
    var i = 0;
    var j = 0;
    var k = 0;
    while (i < elements.length) {
      ...
      var element = elements[i]
      console.log(element)
      if (element.bpmnElement.$type != "bpmn:EndEvent" && element.bpmnElement.$type != "bpmn:SequenceFlow") {
        //element.bounds.x -= 10
        element.bpmnElement.outgoing.forEach(e => {
          while (j < elements.length) {
            //console.log("j")
            //console.log(j)
            var el = elements[j]
            if (e.id == el.bpmnElement.id) {
              el.waypoint.forEach(element => {
                element.x += 20
              });
              j = elements.length
            }
  ...
  /*BaseViewer.prototype.generateElementsClassification = function (flowElements) {
    floeElements.forEach(element => {
      if(element.bpmnElement.$type == '')
    });
  };*/
  ...
  BaseViewer.prototype.importXML = wrapForCompatibility(function importXML(xml, bpmnDiagram) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      self._moddle.fromXML(xml, 'bpmn:Definitions').then(function (result) {
        ...
        console.log("NO SE QUE ONDAAAAAAAAAAAAA")
        console.log(definitions.diagrams[0].plane.planeElement)
  
        self.func3(definitions.diagrams[0].plane.planeElement)
