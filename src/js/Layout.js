...
AutoLayout.prototype._addTreeNodeO = function (flowElement, flowElements) {
...
AutoLayout.prototype._addTreeNode5 = function (flowElement, flowElements) {
    ...
    if (!flowElement.build_marked) {
        ...
        var incoming = getIncomingConnection(flowElement, flowElements);
        if (incoming == 0) {
            console.log("entreeeeeeeeeeeeeeee222222222")
            // root
            this.tree.addParentToNode(node.id, -1);
            flowElements.forEach(element => {
                if (element.id != flowElement.id) {
                    if (!element.build_marked) {
                        element.build_marked = true;
                        var size = getDefaultSize(element);
                        parentNode = this.tree.add(this.nodeCount, element.id, size.width, size.height);
                        this.nodeCount += 1;
                        console.log('Added ' + node.dsc + ' as Node ' + this.nodeCount);
                        self._balanceTreeNodeParentLevels(node);
                        flowElement.nodeId = node.id;
                    } else {
                        parentNode = self.tree.getNodeByName(flowElement.id);
                    }
                    var outgoing = getOutgoingConnection(element, flowElements);
                    if (outgoing != undefined && outgoing.length > 0) {
                        outgoing.forEach(e => {
                            if (!e.build_marked) {
                                console.log("11111111111111111111111")
                                e.build_marked = true;
                                var size = getDefaultSize(e);
                                node = this.tree.add(this.nodeCount, e.id, size.width, size.height);
                                this.nodeCount += 1;
                            } else {
                                console.log("22222222222222222222222")
                                node = self.tree.getNodeByName(flowElement.id);
                            }
                            this.tree.addParentToNode(node.id, parentNode.id)
                            console.log('Added ' + node.dsc + ' as Node ' + this.nodeCount);
                            self._balanceTreeNodeParentLevels(node);
                            flowElement.nodeId = node.id;
                        });
                    }
                    var inc = getOutgoingConnection(element, flowElements);
                    if (inc != undefined && inc.length > 0) {
                        inc.forEach(e => {
                            ...
                            this.tree.addParentToNode(parentNode.id, node.id)
                            console.log('Added ' + node.dsc + ' as Node ' + this.nodeCount);
                            self._balanceTreeNodeParentLevels(parentNode);
                            flowElement.nodeId = node.id;
                        });
                    }
                }
        ...
        console.log("ELLLLLLLLLLLLLLSSSSSSSSSSEEEEEEEE")
        // find and return the node
        return this.tree.getNodeByName(flowElement.id);
    }
};
...
AutoLayout.prototype._nose = function (lista, flowElement, flowElements) {
    var self = this;
    var incoming = getIncomingConnection(flowElement, flowElements);
    if (incoming.length) {
        incoming.forEach(function (connection) {
            ...
            if (source.$type !== 'bpmn:SubProcess') {
                if (source.build_marked) {
                    ...
                    console.log("NOSENOSENOSENOSE")
                    lista.push(source);
                    lista = self._nose(lista, source, flowElements);
                }
            } else {
...
/*
AutoLayout.prototype._nose2 = function (flowElement, flowElements) {
    var self = this;
...
///////////////////////////////////////
///////////////////////////////////////
AutoLayout.prototype._addTreeNode1 = function (flowElement, flowElements) {
    var getDefaultSize = this.DiFactory._getDefaultSize.bind(this.DiFactory);
    var self = this;
    let node;
    var lista = [];
    if (!flowElement.build_marked) {
        ...
        flowElement.build_marked = true;
        lista = [];
        var parentNodes = self._nose(lista, flowElement, flowElements);
        var size = getDefaultSize(flowElement);
        node = this.tree.add(this.nodeCount, flowElement.id, size.width, size.height);
        this.nodeCount += 1;
        if (parentNodes.length === 0) {
